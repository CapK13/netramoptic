...
const Raw7 = () => {
    ...
    fetch('https://netramoptics.onrender.com/fetchData')
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          ...
          console.warn("Expected array from API");
        }
      })
  ...
  const getFilteredFrames = () => {
    return frames
      .filter((frame) => frame.pro_brand?.toLowerCase() === 'raw7')
      .filter((frame) => {
  ...
  return (
    <div className="min-h-screen w-full flex bg-gray-100 transition-all duration-300">
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            {['all', 'men', 'women', 'kids'].map((option) => (
              <button
                ...
                ${genderFilter === option ? 'bg-red-600 text-white border-red-600' : 'bg-gray-800 text-gray-200 border-gray-600 hover:bg-gray-700'}`}
              >
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            {['all', '0-500', '501-1000', '1001-3000', '3001-5000', '5000+'].map((range) => (
              <button
                ...
                className={`px-4 py-2 rounded-full border font-semibold transition duration-300
                ${priceFilter === range ? 'bg-red-600 text-white border-red-600' : 'bg-gray-800 text-gray-200 border-gray-600 hover:bg-gray-700'}`}
              >
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            {['all', 'full_frame', 'half_frame', 'rimless'].map((style) => (
              <button
                ...
                className={`px-4 py-2 rounded-full border font-semibold transition duration-300
                ${styleFilter === style ? 'bg-red-600 text-white border-red-600' : 'bg-gray-800 text-gray-200 border-gray-600 hover:bg-gray-700'}`}
              >
      ...
      <div className="flex-1 flex max-md:mt-20 flex-col items-center py-4 transition-all duration-300">
        ...
        <h1 className="text-2xl font-bold mb-4 text-gray-800">Raw7 Collection</h1>
